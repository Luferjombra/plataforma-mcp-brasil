...
**Status:** **Fase 2 concluída (2026-07-08)** — Fase 1 concluída (2026-07-03) · Validação cruzada concluída (2026-07-07) · Fonte de eventos corporativos resolvida (2026-07-07) · Ajuste por proventos implementado e validado (2026-07-07) · Corte staging → produção concluído (2026-07-08) · Passo 7 (API/frontend/virtualização) e Passo 8 (QA amostral + documentação) concluídos (2026-07-08). Staging continua em operação paralela (ingestão diária + validação cruzada contínua).
**Referenciado em:** `etl/cotahist.py`, `etl/cotahist_backfill.py`, `etl/validar_cotahist.py`, `etl/eventos_corporativos.py`, `etl/aplicar_ajuste_proventos.py`, `etl/promover_cotahist.py`, `backend/routes/rv.py`, `backend/routes/search.py`, `backend/routes/carteira.py`, `backend/postgrest_utils.py`, `frontend/app/rv/page.tsx`, `frontend/app/fundos/page.tsx`, `frontend/app/renda-fixa/page.tsx`, `frontend/lib/api.ts`, `database/migrations/008_cotahist_staging.sql`, `database/migrations/010_eventos_corporativos.sql`, `database/migrations/011_fechamento_adj_staging.sql`, `database/migrations/012_widen_ticker_producao.sql`, `database/migrations/013_rv_variacao_diaria_order.sql`, `.github/workflows/etl.yml`, `qa_run.py`
...
Esse limite de armazenamento é a variável que decide se um universo de ~2.000 tickers é viável no free tier ou exige upgrade para o plano Pro (~$25/mês, 8GB). **Decidido (2026-07-08, ver item 3 da Fase 2 abaixo): manter retenção em 1 ano, sem upgrade.**
...
Revisão de pair-programming (1 rodada, "aprovado com ressalvas") encontrou mais 2 pontos não-bloqueantes, ambos corrigidos: `height: 560` fixo em `/renda-fixa` sem fallback pra viewport curta (trocado por `height: 'min(560px, calc(100vh - 220px))'`) e um flash cosmético de 1 coluna em `/fundos` antes do primeiro measure do `ResizeObserver` (corrigido mostrando o skeleton de loading enquanto `gridWidth === 0`). Um terceiro ponto (espaço vazio abaixo da lista quando a busca filtra para poucos itens, inerente a ter altura de scroll fixa) foi registrado como trade-off aceito, não corrigido. Segunda rodada de revisão: ✅ aprovado.

## Passo 8 — QA e documentação (concluído 2026-07-08)

**E5 — QA amostral estatístico:** `qa_run.py`, seção [3.4], substitui a dependência exclusiva de um único ticker hardcoded (PETR4, mantido como canário barato em 3.3) por uma amostra de até 5 páginas de `GET /rv/ativos` (até ~2.500 ativos do universo de ~2.368) e valida: % de ativos com `preco_atual` não-nulo (≥80%), % de preços em faixa razoável R$0–R$100.000 entre os que têm preço (≥99%), cobertura de preço por `tipo` (ACAO/FII, ≥50% cada — pega regressão de pipeline restrita a um tipo inteiro), e ausência de variações diárias absurdas (>100% em módulo). Testado localmente com `get()`/`check()` mockados cobrindo casos de borda (universo vazio, 100% dos ativos sem preço, variação absurda presente, falha de API na primeira página) — nenhum caminho lança exceção (incluindo divisão por zero quando `com_preco` está vazio), todos reportam falha/sucesso corretamente. Não foi possível rodar contra a API real de produção neste ambiente (política de rede do sandbox bloqueia acesso ao host do Render).

Revisão de pair-programming (1 rodada, "aprovado com ressalvas") apontou que os 4 limiares (80%/99%/50%/50%) nunca foram validados contra dados reais, e que `qa_run.py` alimenta o gate de bloqueio de `.github/workflows/qa.yml` (score < 90% falha o workflow) — um limiar errado bloquearia PRs futuros sem motivo real. Corrigido: os 4 checks de limiar usam um novo `check_info()` (informativo, não entra no score) até rodar uma vez via `workflow_dispatch` e confirmar os números reais; só o check de "amostra coletada" (o endpoint responde e devolve dados) continua bloqueante. Também corrigido um comentário que citava ELET3/RBRF11 como exemplo de tickers que ficariam sem `preco_atual` — na verdade esses são preservados pela precedência por `fonte` do corte (permanecem com dado fresco da brapi); o grupo real em risco são os ~2.338 tickers só-COTAHIST sem fallback de brapi.

**Documentação:** `README.md` (tabela de volumes de dados atualizada para refletir o universo pós-corte — `rv_ativos`/`rv_historico` combinando brapi.dev + COTAHIST/B3, roadmap marcando Fase 2 como concluída), `architecture.md` (fluxo macro e tabela de tabelas atualizados para refletir que `rv_ativos`/`rv_historico` de produção agora recebem dados de duas fontes, staging em operação paralela pós-corte), e este ADR (status geral marcado como Fase 2 concluída).
...
- `database/migrations/013_rv_variacao_diaria_order.sql` — `ORDER BY` determinístico em `rv_variacao_diaria()` + `ticker` VARCHAR(12)
...
- `backend/routes/rv.py` — `GET /ativos` com paginação/busca server-side (E2), `backend/postgrest_utils.py::sanitizar_busca`
- `backend/routes/carteira.py` — `GET /posicoes` reaproveitando `rv_variacao_diaria()` (E3)
- `frontend/app/rv/page.tsx`, `frontend/app/fundos/page.tsx`, `frontend/app/renda-fixa/page.tsx` — virtualização de listas (P6)
- `qa_run.py` — QA amostral estatístico do universo RV (E5)
