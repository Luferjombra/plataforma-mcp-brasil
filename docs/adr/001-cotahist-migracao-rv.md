...
5. **Investigar ELET3 e RBRF11 (diagnosticado 2026-07-08, ainda não resolvido)** — `validar_cotahist.py` agora reporta o range de datas de cada fonte quando não há overlap, o que revelou o padrão real: as janelas são **completamente disjuntas no tempo**, não uma diferença de cobertura. ELET3: produção (brapi) cobre 2026-03-18→2026-07-07, staging (COTAHIST) cobre 2025-07-07→**2025-11-07** — nenhuma linha depois disso. RBRF11: produção 2026-03-18→2026-07-06, staging 2025-07-07→**2025-10-02**. Há um gap de 4-5,5 meses sem registro em nenhuma das duas fontes. Como o staging tem cron diário rodando continuamente desde então, a ausência de linhas não é falta de coleta — o COTAHIST simplesmente parou de ter negociação sob esses códigos exatos naquele período. **Hipótese mais provável:** rebatização de ticker ou evento societário (fusão de FII no caso de RBRF11; reestruturação/especificação no caso de ELET3 ON) — o brapi parece resolver/normalizar o código antigo para os dados recentes, o arquivo bruto da B3 não. **Próximo passo:** buscar em `rv_ativos_staging` um ticker candidato a sucessor (nome parecido, primeira data logo após o corte de cada um) para confirmar a hipótese antes de decidir como tratar esses 2 papéis no corte de produção.
