...
def buscar_historico(tabela: str, ticker: str, cutoff: str) -> dict:
    """
    Retorna {data: {fechamento, volume}} para um ticker numa tabela, só a
    partir de `cutoff`. Sem esse filtro, tickers com >1000 registros batem
    no limite padrão do PostgREST e a consulta volta truncada (e sem
    garantia de quais linhas vêm — pode não cobrir a janela recente).
    """
    res = (
